...
const AvatarEditor: React.FC<EditorProps> = ({ template, onGenerate, isGenerating, heyGenKey, userCredits }) => {
    // ... (AvatarEditor implementation unchanged)
...
export const Editor: React.FC<EditorProps> = (props) => {
    const { template, onBack, onGenerate } = props;
    const [currentTemplate, setCurrentTemplate] = useState<Template>(template);
    const [prefilledData, setPrefilledData] = useState<any>(null);

    // Handle incoming data from Clipper to bridge to Shorts Maker
    const handleClipperGenerate = async (data: any) => {
        if (data.type === 'CLIPPING' && data.shouldRedirect && data.idea) {
            // SWITCH MODE: Clipper -> Shorts Maker
            const shortsTemplate: Template = {
                id: 'shorts_generated',
                name: 'Shorts Maker',
                thumbnailUrl: '',
                category: 'AI',
            ...
            setCurrentTemplate(shortsTemplate);
            setPrefilledData({ idea: data.idea });
        } else {
            // Normal behavior
    ...
    // Pass prefilled data (script idea) to ShortsMaker if available
    const shortsMakerProps = prefilledData ? { ...props, template: currentTemplate, initialIdea: prefilledData.idea } : { ...props, template: currentTemplate };

    // Route to specialized editors based on template mode
    if (currentTemplate.mode === 'CLIPPING') {
        content = <ClippingEditor {...props} onGenerate={handleClipperGenerate} />;
    } else if (currentTemplate.mode === 'AUDIOBOOK') {
        content = <AudiobookEditor {...props} />;
    } else if (currentTemplate.mode === 'IMAGE_TO_VIDEO' || currentTemplate.mode === 'UGC_PRODUCT') {
        content = <ImageToVideoEditor {...props} />;
    } else if (currentTemplate.mode === 'FASHION_SHOOT' || currentTemplate.mode === 'TEXT_TO_IMAGE' || currentTemplate.mode === 'IMAGE_TO_IMAGE') {
        content = <FashionShootEditor {...props} />;
    } else if (currentTemplate.mode === 'SHORTS' || currentTemplate.mode === 'STORYBOOK') {
        // Need to use modified props for ShortsMaker to accept initialIdea
        // We do this by casting/augmenting the props since ShortMakerEditor handles it internally
        // (We need to update ShortMakerEditor to accept initialIdea prop first, done below conceptually)
        content = <ShortMakerEditor {...shortsMakerProps} />;
    } else {
