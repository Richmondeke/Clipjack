...
export const getUserProfile = async (userId: string, retries = 3): Promise<UserProfile | null> => {
  if (!isSupabaseConfigured()) {
    const user = getMockUser();
  ...
  try {
    ...
    if (error) {
      ...
      if (retries > 0 && (
          ...
          console.warn(`Network error fetching profile, retrying... (${retries} attempts left)`);
      ...
      // Handle missing profile or other errors by fetching basic auth user data
      if (error.code === 'PGRST116' || error.code === '42P01' || error.message?.includes('Failed to fetch')) {
        try {
            ...
            // Fallback if auth check also fails
    ...
    return {
      ...
      // Catch any synchronous errors or fetch throws
      if (retries > 0 && (err.message?.includes('Failed to fetch') || err.name === 'TypeError')) {
          console.warn(`Exception fetching profile, retrying... (${retries} attempts left)`);
          await wait(1000);
          return getUserProfile(userId, retries - 1);
      }
